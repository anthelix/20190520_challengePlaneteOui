...
#------features cat
# find the uniq value 'year'

liste_year = []
for x in Xtrain['year']:
    if x not in liste_year:
       liste_year.append(x)
categorical_features = ['year', 'month', 'hours', 'season']
for var in categorical_features:
    Xtrain[var] = Xtrain[var].astype('category')
Xtrain_cat = Xtrain.select_dtypes(include='category')
Xtrain.dtypes
list(Xtrain.columns)

colT = ColumnTransformer([("dummy", OneHotEncoder(sparse=False),[8,9,10,11,13])])
Xtrain_cat_result = colT.fit_transform(Xtrain)
